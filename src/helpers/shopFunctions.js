...
export const calTotal = () => {
  ...
  if (totalCartValue === 0) {
    ...
    cartProducts.forEach((product) => {
      const price = product.querySelector('.product__price__value').textContent;
      totalCartValue += Number(price);
    });
    const totalCartValueElement = document.querySelector('.total-price');
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    ...
    saveCartID(id);
    const createCart = createCartProductElement(product);
